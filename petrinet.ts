...
import { Places, Transition, Transitions, TransitionCallback, CycleCallback, InputType } from './definitions'
...
export class PetriNet {
  ...
  private executeTransition(transition: Transition) {
    ...
    inputs.forEach(({ place, multiplicity, type }) => {
      const newTokensAmount = this.getNewTokensAmount(type, multiplicity, this.places[place].tokens);
